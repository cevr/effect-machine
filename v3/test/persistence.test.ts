    ...
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const result = yield* Effect.exit(system.restoreAll(noTypeMachine));

          expect(result._tag).toBe("Failure");
          if (result._tag === "Failure") {
            const err = Cause.failureOption(result.cause);
            expect(Option.isSome(err) && err.value._tag).toBe("PersistenceError");
            if (Option.isSome(err)) {
              expect(err.value.message).toContain("machineType");
            }
          }
        }).pipe(Effect.provide(sharedLayer)),
      );
